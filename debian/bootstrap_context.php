...
if (!is_dir($ajaxplorerCacheFolder)) {
    try {
        mkdir($ajaxplorerCacheFolder);
    } catch (\Exception $e) {
        die("Unable to create the cache folder : " . $e->getMessage());
    }
}

if (!is_dir($ajaxplorerDataFolder)) {
    try {
        mkdir($ajaxplorerDataFolder);
    } catch (\Exception $e) {
        die("Unable to create the cache folder : " . $e->getMessage());
    }
}
...
define("AJXP_DATA_PATH", $ajaxplorerDataFolder);
define("AJXP_CACHE_DIR", $ajaxplorerCacheFolder);
define("AJXP_FORCE_LOGPATH", dirname(\ConfigIgestisGlobalVars::logFile()));
